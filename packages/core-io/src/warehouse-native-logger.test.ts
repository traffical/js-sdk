...
/**
 * The sample deliberately gives `policyKey`/`allocationKey` values that differ
 * from `policyId`/`allocationName`, so a regression that drops them (or falls
 * back to the opaque id) fails instead of coincidentally matching.
 */
...
describe("createWarehouseNativeLoggerPlugin", () => {
  ...
  test("All AssignmentLogEntry fields mapped correctly", () => {
    ...
    const entry = sampleEntry({
      ...
      policyKey: "checkout-price-test",
      allocationName: "control",
      allocationKey: "control-key",
    ...
    expect(track.mock.calls[0]?.[1]).toEqual({
      ...
      // The stable keys a warehouse assignment definition joins on.
      policy_key: "checkout-price-test",
      allocation_name: "control",
      allocation_key: "control-key",
      timestamp: "2025-06-15T12:30:00.000Z",
  ...
  test("row shape matches the PHP/Python warehouse row: stable keys are present", () => {
    const track = mock(() => {});
    createWarehouseNativeLoggerPlugin({
      destination: { type: "segment", analytics: { track } },
    })(sampleEntry());
    const row = track.mock.calls[0]?.[1] as Record<string, unknown>;
    // Guards the defect this factory shipped with: `policy_key` and
    // `allocation_key` were absent, so BYO warehouse joins matched nothing.
    expect(row).toHaveProperty("policy_key");
    expect(row).toHaveProperty("allocation_key");
    expect(row.policy_key).not.toBe(row.policy_id);
    expect(row.allocation_key).not.toBe(row.allocation_name);
  });

  test("absent stable keys stay undefined rather than falling back to ids", () => {
    const track = mock(() => {});
    createWarehouseNativeLoggerPlugin({
      destination: { type: "segment", analytics: { track } },
    })(sampleEntry({ policyKey: undefined, allocationKey: undefined }));
    const row = track.mock.calls[0]?.[1] as Record<string, unknown>;
    expect(row.policy_key).toBeUndefined();
    expect(row.allocation_key).toBeUndefined();
  });
...
describe("createWarehouseNativeLogger", () => {
  ...
  test("jitsu s2s builds the default server-to-server URL and sends the write key", () => {
    const fetchImpl = mock(async () => ({ ok: true, status: 200 }) as unknown as Response);
