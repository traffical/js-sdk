 ...
 * assignment logger.
 *
 * Lives in `@traffical/core-io` (not `@traffical/core`) because the Jitsu
 * destination performs `fetch` I/O, and in core-io rather than `js-client` so
 * that every SDK built on it — including `@traffical/node`, the server-side
 * BYO-warehouse path — gets the same implementation instead of re-deriving it.
 */
...
function toAssignmentProps(entry: AssignmentLogEntry): Record<string, unknown> {
  return {
    ...
    // `policy_key` / `allocation_key` are the STABLE identifiers a warehouse
    // assignment definition joins on — the `*_id` columns are opaque and match
    // nothing. Omitting them (as this factory once did) yields rows that join
    // to zero policies, so metrics come back empty rather than erroring.
