...
/**
 * Base constraint for event property maps.
 * Generated by `traffical generate-types` as `TrafficalEventProperties`.
 * When not provided, falls back to untyped Record<string, unknown>.
 */
export type TrackEventMap = Record<string, Record<string, unknown>>;

/**
 * Type-safe track function signature.
 * When TEvents is provided (from codegen), event names and properties are constrained.
 * When TEvents is the default (TrackEventMap), the signature is the same as today.
 */
export type TypedTrackFn<TEvents extends TrackEventMap = TrackEventMap> =
  <E extends Extract<keyof TEvents, string>>(
    event: E,
...
/** A single schema violation reported by the edge validator. */
...
/** Schema warning for a single event in a batch. */
export interface EventSchemaWarning {
  index: number;
  event: string;
  violations: SchemaViolation[];
}

/** Batch event response from the edge worker. */
export interface EventBatchResponse {
  accepted: number;
  pipelineWritten?: number;
  validationErrors?: Array<{ index: number; error: string }>;
  schemaWarnings?: EventSchemaWarning[];
}

/**
 * Callback for schema validation warnings (dev-mode).
 * Receives warnings from the edge response after each batch flush.
 */
export type OnSchemaWarnings = (warnings: EventSchemaWarning[]) => void;
